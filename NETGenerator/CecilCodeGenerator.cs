...
namespace PascalABCCompiler.NETGenerator
{
    ...
    /// </summary>
    public class CecilCodeGenerator : AbstractVisitor, ICodeGenerator
    {
        ...
        public CecilCodeGenerator(Hashtable StandartDirectories)
        {
        ...
        private void BuildDotnetNative(string orig_dir, string dir, string publish_dir, string SourceFileName)
        {
            ...
            {
                throw new SaveAssemblyError(ex.Message, new   PascalABCCompiler.TreeRealization.location(0, 0, 0, 0, new   PascalABCCompiler.TreeRealization.document(SourceFileName)));
            }
        }
        ...
        public void ConvertFromTree(  IProgramNode p, string TargetFileName, string SourceFileName, CompilerOptions options, string[] ResourceFiles)
        {
            ...
            if (options.MainResourceFileName != null)
            {
                ...
                {
                    throw new SourceFileError(options.MainResourceFileName);
                }
            }
                    ...
                    try
                    {
                        if (comp_opt.target == TargetType.Exe || comp_opt.target == TargetType.WinExe)
                        {
                            if (IsDotnet5() || IsDotnetNative())
                                ab.Save(an.Name + ".dll");
                            else if (comp_opt.platformtarget == CompilerOptions.PlatformTarget.x86)
        ...
        private void CloseTypes()
        {
            ...
            for (int i = 0; i < types.Count; i++)
                if (types[i].IsInterface)
                    ...
                    catch(TypeLoadException ex)
                    {
                        ...
                        {
                              ICommonTypeNode ctn = helper.GetTypeNodeByTypeBuilder(types[i]);
                            if (ctn != null)
            ...
            for (int i = 0; i < value_types.Count; i++)
            {
                ...
                {
                      ICommonTypeNode ctn = helper.GetTypeNodeByTypeBuilder(value_types[i]);
                    if (ctn != null)
                    {
            ...
            for (int i = 0; i < failed_types.Count; i++)
                ...
                {
                      ICommonTypeNode ctn = helper.GetTypeNodeByTypeBuilder(failed_types[i]);
                    if (ctn != null)
                        throw new PascalABCCompiler.Errors.CommonCompilerError(ex.Message, ctn.Location.document.file_name, ctn.Location.begin_line_num, ctn.Location.begin_column_num);
                    else
                        throw ex;
        ...
        private void MakeAttribute(ICommonFunctionNode func)
        {
            ...
            for (int i = 0; i < attrs.Length; i++)
            {
                ...
                ((attrs[i].AttributeConstructor is ICompiledConstructorNode) ? (attrs[i].AttributeConstructor as ICompiledConstructorNode).constructor_info : helper.GetConstructor(attrs[i].AttributeConstructor).cnstr, get_constants(attrs[i].Arguments),
                    get_named_properties(attrs[i].PropertyNames), get_constants(attrs[i].PropertyInitializers),
                    get_named_fields(attrs[i].FieldNames), get_constants(attrs[i].FieldInitializers));
                if (attrs[i].qualifier ==   attribute_qualifier_kind.return_kind)
                {
        ...
        private void ConvertFunctionHeader(ICommonFunctionNode func)
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                if (default_value != null)
                {
                    ...
                    if (default_value is   PascalABCCompiler.TreeRealization.null_const_node) // SSM 20/04/21
                        pb.SetConstant(null);
        ...
        internal void GenerateInitCodeForClassConstant(  IClassConstantDefinitionNode value)
        {
            TypeInfo ti = helper.GetTypeReference(value.type), cur_ti = helper.GetTypeReference(value.comperehensive_type);
        ...
        public override void visit(  ICompiledStaticMethodCallNode value)
        {
            ...
            for (int i = 0; i < real_parameters.Length; i++)
            {
                ...
                if (ctn2 != null && !(real_parameters[i] is   INullConstantNode) && (ctn3.is_value_type || ctn3.is_generic_parameter) && (ctn2.compiled_type == TypeFactory.ObjectType || ctn2.IsInterface))
                    il.Emit(OpCodes.Box, helper.GetTypeReference(ctn3).tp);
                else if (ctn2 != null && !(real_parameters[i] is   INullConstantNode) && ctn4 != null && (ctn4.is_value_type || ctn4.is_generic_parameter) && (ctn2.compiled_type == TypeFactory.ObjectType || ctn2.IsInterface))
        ...
        private void EmitArguments(IParameterNode[] parameters, IExpressionNode[] real_parameters)
        {
            ...
            for (int i = 0; i < real_parameters.Length; i++)
            {
                ...
                //Дополнительная проверка введена именно для этого.
                bool box_awaited =
                    (ctn2 != null && (ctn2.compiled_type == TypeFactory.ObjectType || ctn2.compiled_type == TypeFactory.EnumType) || tn2.IsInterface) && !(real_parameters[i] is   INullConstantNode) 
        ...
        protected virtual void EmitOperator(IBasicFunctionCallNode fn)
        {
            ...
            switch (ft)
            {
                ...
                case basic_function_type.objtoobj:
                {
                    ...
                    if (!(par0 is   INullConstantNode) && (tn.is_value_type || tn.is_generic_parameter))
                    {
        ...
        public override void visit(  ISimpleArrayIndexingNode value)
        {
            ...
            if (string_getter)
            {
                ...
                if (copy_string)
                {
                    /*var mcall = new   PascalABCCompiler.TreeRealization.compiled_static_method_call(  PascalABCCompiler.TreeRealization.compiled_function_node.get_compiled_method(TypeFactory.StringCopyMethod), null);
                    mcall.parameters.AddElement(value.array as   PascalABCCompiler.TreeRealization.expression_node);
